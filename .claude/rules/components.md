...
## Hooks (webapp/lib/hooks/)

| Hook | File | Purpose |
|------|------|---------|
| `useNativeApp` | `hooks/useNativeApp.ts` | isNative 플래그 + haptic/share/toast/clipboard (웹 폴백 포함). **신규 코드에서는 useIsNativeApp 대신 이것을 사용.** |
| `useIsNativeApp` | `hooks/useIsNativeApp.ts` | boolean만 반환 — 레거시. 신규 코드에서는 useNativeApp 사용 |

## Native Bridge (webapp/lib/bridge.ts)

싱글톤 NativeBridge 인스턴스. 주요 메서드:
- `bridge.isNativeApp()` — 네이티브 앱 감지
- `bridge.haptic(pattern)` — 진동 피드백
- `bridge.share(title, url)` — 네이티브 공유 시트
- `bridge.toast(message)` — 네이티브 토스트
- `bridge.call(action, params)` — 결과를 반환하는 네이티브 액션
- `bridge.fire(action, params)` — fire-and-forget 네이티브 액션
- `bridge.sendAuth(token)`, `bridge.sendLogout()` — 인증 브릿지
