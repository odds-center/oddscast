...
interface CardProps {
  ...
  style?: ViewStyle;
  variant?: 'default' | 'base' | 'elevated' | 'outlined' | 'compact';
  onPress?: () => void;
}
...
export const Card: React.FC<CardProps> = ({ children, style, variant = 'default', onPress }) => {
  const effectiveVariant = variant === 'base' ? 'default' : variant;
  const cardStyle = [styles.card, styles[effectiveVariant], style];
...
export const Section: React.FC<CardProps> = ({ children, style, variant = 'default' }) => {
  const effectiveVariant = variant === 'base' ? 'default' : variant;
  return (
    <View style={[styles.card, styles[effectiveVariant], styles.section, style]}>{children}</View>
  );
};
