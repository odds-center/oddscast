...
**위치:** `server/src/common/race-includes.ts` (또는 동일 역할의 include 유틸)
...
async getOrCreateRace(meet: string, rcDate: string, rcNo: string): Promise<Race | null> {
  // TypeORM: raceRepository.upsert 또는 findOne + save
  return this.raceRepository.upsert({ meet, rcDate, rcNo, status: 'SCHEDULED' }, ['meet', 'rcDate', 'rcNo']);
}
```
...
- [DATABASE_SCHEMA.md](../architecture/DATABASE_SCHEMA.md) — TypeORM·DB 스키마
