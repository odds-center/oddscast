...
| 페이지 | 라우트 | 목적 | C1~C10 체크 | 현황 | 비고/계획 |
|--------|--------|------|-------------|------|-----------|
| **로그인** | `/auth/login` | 이메일/비밀번호, 로그인 후 리다이렉트 | C1,C6,C8,C9 | ✅ | 에러(root)·필수 검증, BackLink |
| **회원가입** | `/auth/register` | 이메일/비밀번호/이름·닉네임, 가입 후 로그인 | C1,C6,C8,C9 | ✅ | 동일, minLength 검증 |
| **비밀번호 찾기** | `/auth/forgot-password` | 이메일 입력·재설정 메일 발송 | C1,C6,C7,C8,C9 | ✅ | 성공 시 안내, mutation 에러 표시 |
| **비밀번호 재설정** | `/auth/reset-password` | 토큰·새 비밀번호 | C1,C6,C7,C8,C9 | ✅ | 토큰 없음 안내, API 에러(root) |
