...
until docker exec goldenrace-mysql-dev mysqladmin ping -h localhost -u root -prootpassword --silent; do
...
echo "📝 데이터베이스 완전 초기화..."
docker exec -i goldenrace-mysql-dev mysql -u root -p'rootpassword' < mysql/init/01_create_database.sql

echo "✅ 데이터베이스 초기화가 완료되었습니다!"

# 테이블 목록 확인
echo "📋 생성된 테이블 목록:"
docker exec -i goldenrace-mysql-dev mysql -u $DB_USER -p$DB_PASSWORD $DB_NAME -e "SHOW TABLES;"
