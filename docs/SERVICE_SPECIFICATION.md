...
| 3 | **AI 예측 (개별)** | RACE 티켓 사용 시 실시간 KRA 재조회 + Gemini 분석 (마체중·날씨 반영) | WebApp, Mobile | 로그인 |
...
| 9 | **종합 예측권 (MATRIX)** | 구독 포함 (5천원당 1장) + 개별 구매 1,000원/장, 일일 종합 예상표 전체 열람 (배치 사전 캐싱, 전 유저 동일) | WebApp, Mobile | 로그인 |
| 10 | ~~**포인트**~~ | ~~예측권 구매에 사용~~ **제거됨** — Points 모듈 전체 삭제 | — | — |
...
| **Mobile** | WebView로 WebApp 로드 (Native 로그인·푸시) | React Native CLI |
...
| 구분 | 기술 |
|------|------|
| **클라이언트** | Next.js (WebApp), React Native CLI (Mobile), Next.js (Admin) |
