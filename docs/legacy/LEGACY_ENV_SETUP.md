...
OddsCast는 `.env` 파일 대신 **시스템 환경변수**를 사용합니다.
...
```bash
# PostgreSQL Database (레거시: SUPABASE_DB_* 변수명 유지)
...
```bash
# ~/.zshrc 파일에 추가
echo 'export SUPABASE_DB_HOST=your-db-host.example.com' >> ~/.zshrc
...
```bash
# ~/.bashrc 파일에 추가
echo 'export SUPABASE_DB_HOST=your-db-host.example.com' >> ~/.bashrc
source ~/.bashrc
```
...
```bash
SUPABASE_DB_HOST      # PostgreSQL DB 호스트 (레거시 변수명)
