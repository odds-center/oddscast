...
> **현재 상태:** Prisma 제거 완료. DB 접근은 `pg` Pool + `PgService` + **raw SQL** 로만 구현됨.  
> **목표:** raw SQL을 **TypeORM** 으로 전환하여 유지보수성·타입 안전성·마이그레이션 도구를 확보한다.
...
- **현재 DB 레이어:** `server/src/database/pg.service.ts`, `server/src/database/database.module.ts`
...
4. **호환 유지** — 전환 중에는 `PgService`와 TypeORM을 병행 가능. 한 모듈 전환 완료 후 해당 모듈에서만 TypeORM 사용.
...
- [x] **kra** — `KraService` / `KraController`에서 PgService 제거, DataSource + query 래퍼로 raw SQL 유지
- [ ] **payments / subscription / billing** — 해당 테이블 Entity 및 서비스 전환
- [ ] **admin / config** — 나머지 Entity 및 쿼리 전환

### 4.4 PgService 제거

- [ ] 모든 서비스가 TypeORM만 사용하도록 전환 후 `PgService` 사용처 제거
- [ ] `DatabaseModule`에서 `PgService` 제거, 필요 시 TypeORM `DataSource`만 노출
- [ ] `docs/PRISMA_REMOVAL.md`에 “Phase 2: TypeORM 전환 완료” 내용 반영

### 4.5 마이그레이션

- [ ] 초기 스키마를 TypeORM migration으로 생성 (기존 DB와 동기화)
- [ ] 이후 스키마 변경은 `typeorm migration:generate`로 생성 후 `migration:run`으로 적용
- [ ] `docs/guides/TYPEORM_SETUP.md`에 CI/배포 시 마이그레이션 실행 방법 명시
