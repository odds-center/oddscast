...
> **정리:** “마이그레이션”은 이 단계에서 완료된 상태이며, 선택 작업만 남음(§4.4 PgService 제거, §4.5 migration CLI 도입).
...
- [ ] (선택) TypeORM migration CLI 도입 시: 초기 migration 생성 후 migration:run, 이후는 migration:generate → migration:run
