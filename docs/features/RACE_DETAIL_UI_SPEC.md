...
**KRA 출전마**: 경주 **전** KRA 출전표 API(API26_2)에서 받는 "출전 예정 말" 정보 (마명, 기수, 부담중량, 레이팅 등).

- **경주 전**: AI 예측·관전 판단에 핵심 데이터 — 출전마 섹션 표시
- **경기 종료 후**: 출전마 섹션 **숨김** (`status=COMPLETED` 또는 `view=result`). 경주 결과 테이블만 표시
...
|------|-----|------|
| 순위 | ord | 도착 순위 |
| 출전번호 | chulNo | 해당 경주 출전번호(게이트) |
| 마명 | hrName | 말 이름 (hrNo는 UI 미표기) |
| 기수 | jkName | 기수명 |
| 조교사 | trName | 조교사명 |
