...
### 테마 — Turf Green + Warm Amber

| 구분 | 값 | 비고 |
|------|-----|------|
| 배경 | `#f8faf8` | background |
| 카드 | `#ffffff` | card |
| Primary | `#16a34a` | turf green |
| Primary Dark | `#15803d` | hover/active |
| Accent | `#d97706` | warm amber |
| 텍스트 (주) | `#0f1a0f` | foreground |
| 텍스트 (보조) | `text-secondary` | CSS variable |
| 텍스트 (약) | `text-tertiary` | CSS variable |
| 테두리 | `#e2e8e2` | border |
| 에러 | `#b91c1c` | error |
| 성공 | `#15803d` | success |
| 경고 | `#a16207` | warning |
...
## 뒤로가기 (CompactPageTitle)

> `BackLink` 컴포넌트는 제거됨. 모든 뒤로가기는 `CompactPageTitle`의 뒤로가기 버튼으로 처리.

### 규칙

- **`CompactPageTitle`**: 모바일 sticky GNB 스타일 뒤로가기 + 제목. `backHref` prop으로 목적지 지정
- 별도 BackLink 컴포넌트 사용 금지
...
경마 용어를 처음 접하는 사용자에게 맥락 설명을 제공하는 툴팁.
- **shadcn Tooltip** (`@/components/ui/tooltip`): Radix UI 기반. 복잡한 인터랙션에 적합.
- **SimpleTooltip** (`@/components/ui/SimpleTooltip`): CSS 기반 경량 버전. 테이블 헤더 등.

```tsx
import { SimpleTooltip } from '@/components/ui/SimpleTooltip';

<SimpleTooltip content='산지(한/미/일) + 연령 + 성별(수/암/거)' inline>마령</SimpleTooltip>
```
...
| page/ | FilterDateBar, FilterChips, Pagination              | `@/components/page/` |
...
| ui/   | Button, Badge, Card, Input, Switch, Select (shadcn) | `@/components/ui`    |
| ui/   | Dialog, Tabs, Label, Separator, Alert, Skeleton, Table (shadcn) | `@/components/ui` |
| ui/   | Toggle, SectionTitle, SimpleTooltip, DataTable, LinkCard | `@/components/ui` |
...
한국어 텍스트는 모바일(320-428px)에서 자연 줄바꿈 위치가 어색할 수 있으므로 **의도적 개행**을 사용한다.

- **`whitespace-pre-line`**: `EmptyState`, `SectionCard` description, `DateHeader` subtitle에 기본 적용됨
- **`\n` 개행**: 25자 이상 한국어 문장이 두 절로 나뉠 때 (예: "경기가 종료되었습니다.\n결과를 확인하세요.")
- **`<br className='sm:hidden' />`**: 모바일에서만 개행, 데스크톱은 한 줄 유지 (예: 이용 안내 항목)
- **적용 기준**: 25자 이상 문장에서 자연스러운 의미 단위로 분리 (마침표 뒤, 조사 뒤)
- **미적용 대상**: 20자 이하 짧은 텍스트, 버튼 라벨, 탭 이름 등

적용된 곳: dateHeaderMessages.ts, EmptyState descriptions, matrix 힌트/CTA, 티켓 구매 안내, 회원탈퇴 dialog
