...
| **구독명**      | AI 예측권 프리미엄                           |
| **가격**        | 19,800원/월                                  |
| **포함 내용**   | 월 24장의 AI 예측권 (기본 20장 + 보너스 4장) |
| **장당 단가**   | 825원/장                                     |
| **할인율**      | 25% 할인                                     |
| **절약액**      | 개별 구매 대비 월 4,200원                    |
| **결제 방식**   | 정기 결제 (자동 갱신)                        |
...
| **구독명**      | AI 예측권 라이트                             |
| **가격**        | 9,900원/월                                   |
| **포함 내용**   | 월 11장의 AI 예측권 (기본 10장 + 보너스 1장) |
| **장당 단가**   | 900원/장                                     |
| **할인율**      | 18% 할인                                     |
| **절약액**      | 개별 구매 대비 월 1,100원                    |
| **결제 방식**   | 정기 결제 (자동 갱신)                        |
| **예측 방식**   | LLM AI (GPT-4, Claude 등)                    |
| **예측 정확도** | 평균 70%+ (목표)                             |
| **상태**        | ✅ 구현 완료                                 |
...
> **💡 팁**:
>
> - 월 11장 이상 사용하시면 라이트 구독이 더 저렴합니다!
> - 월 24장 이상 사용하시면 프리미엄 구독이 더 저렴합니다!
> - 프리미엄 구독 시 장당 825원으로 **25% 할인**!
...
class PredictionTicketService {
   ...
   * - Premium: 35장
   */
  async issueMonthlyTickets(userId: string, subscriptionId: string, planId: 'LIGHT' | 'PREMIUM') {
    const tickets = [];
    const today = new Date();
    const expiryDate = new Date(today.setMonth(today.getMonth() + 1));

    // 플랜별 발급 수량
    const ticketCount = planId === 'PREMIUM' ? 35 : 15;
