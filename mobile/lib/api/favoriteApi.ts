...
export class FavoriteApi {
  ...
  async getFavorite(favoriteId: string): Promise<Favorite> {
    try {
      const response = await apiClient.get<ApiResponse<Favorite>>(`${this.baseUrl}/${favoriteId}`);
      return handleApiResponse(response);
    } catch (error) {
      throw handleApiError(error);
    }
  ...
  async createFavorite(favoriteData: CreateFavoriteRequest): Promise<Favorite> {
    try {
      const response = await apiClient.post<ApiResponse<Favorite>>(this.baseUrl, favoriteData);
      return handleApiResponse(response);
    } catch (error) {
      throw handleApiError(error);
    }
  ...
  async getFavoriteStatistics(): Promise<FavoriteStatistics> {
    try {
      const response = await apiClient.get<ApiResponse<FavoriteStatistics>>(
  ...
  async bulkDeleteFavorites(favoriteIds: string[]): Promise<{
    deleted: number;
    failed: number;
    errors: Array<{ favoriteId: string; error: string }>;
  }> {
    try {
