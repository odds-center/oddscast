# 📋 GOLDEN RACE 서비스 명세서 (Service Specification)
...
**GOLDEN RACE**는 **AI 기반 경마 예측 정보를 제공하는 구독형 콘텐츠 서비스**입니다.
...
| 6 | **내가 고른 말** | 승식별 선택 기록, 적중 시 포인트 | WebApp, Mobile | 로그인 |
| 7 | **즐겨찾기** | 관심 경주 저장 (RACE만) | WebApp, Mobile | 로그인 |
| 8 | **예측권** | 구독·포인트·개별 구매로 획득 | WebApp, Mobile | 로그인 |
| 9 | **포인트** | 적중 시 지급, 예측권 구매에 사용 | WebApp, Mobile | 로그인 |
...
#### 내가 고른 말 (Picks)
- **목적**: 승식별로 선택한 말 기록, 적중 시 포인트 지급
- **승식**: SINGLE, PLACE, QUINELLA, EXACTA, QUINELLA_PLACE, TRIFECTA, TRIPLE
- **사행성**: 없음 (가상 포인트만, 예측권 구매용)
- **라우트**: `/mypage/picks`
- **API**: `POST /api/picks`, `GET /api/picks/race/:raceId`

#### 즐겨찾기
- **대상**: RACE(경기)만 지원
- **동작**: 토글 (추가/삭제)
- **라우트**: `/mypage/favorites`
...
4. 내가 고른 말 저장 → 적중 시 포인트 지급 → 포인트로 추가 예측권 구매
```

### 4.3 무구독 사용자

```
1. 내가 고른 말 저장 → 적중 시 포인트 지급
2. 포인트 1200pt 이상 → 예측권 1장 구매
3. 예측권으로 특정 경주 전체 분석 열람
```
