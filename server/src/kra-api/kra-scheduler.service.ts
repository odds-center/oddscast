...
export class KraSchedulerService {
    ...
    try {
      ...
      for (
        ...
        if (racePlans && racePlans.length > 0) {
          this.logger.log(
            `Collected ${racePlans.length} race plans for ${meet.name} on ${dateStr}`
          );

          // 데이터베이스에 저장 (구현 필요)
          await this.saveRacePlansToDatabase(racePlans, meet);
        }
   ...
   * 경주계획표를 데이터베이스에 저장
   */
  private async saveRacePlansToDatabase(racePlans: any[], meet: KraMeetInfo) {
    try {
