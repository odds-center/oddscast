...
export default function AccuracyPreviewSection() {
  ...
  return (
    ...
    >
      {isLoading ? (
        <div className='py-8 text-center text-stone-400 text-sm'>준비 중...</div>
      ) : error ? (
        <div className='py-6 text-center'>
          <p className='text-sm text-text-secondary'>통계를 불러올 수 없습니다.</p>
          <Button type='button' variant='outline' size='sm' onClick={() => refetch()} className='mt-2'>
            다시 시도
          </Button>
        </div>
      ) : !overall || overall.totalCount === 0 ? (
        <div className='py-8 text-center text-stone-400 text-sm'>아직 집계된 예측 데이터가 없습니다.</div>
      ) : (
