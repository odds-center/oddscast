...
> **스키마:** DB는 `docs/db/schema.sql` 수동 적용. TypeORM `synchronize`는 `false`, migration CLI는 선택 사항.  
...
| **스키마 드리프트** | DB 스키마와 코드가 어긋나기 쉽고, 문서(docs/db/schema.sql)와 실제 쿼리가 달라질 수 있음. |
...
- **스키마 참고:** `docs/db/schema.sql` — 테이블·컬럼·제약·Enum 정의
...
- **Prisma 제거:** 완료. DB 접근은 TypeORM 전용, 스키마는 `docs/db/schema.sql`.

---

## 3. TypeORM 전환 원칙

1. **Entity 우선** — `docs/db/schema.sql` 및 기존 Enum을 기준으로 Entity 정의. 한 번에 전체가 아닌 **모듈 단위**로 전환.
...
- [x] 나머지 Entity: Race, RaceEntry, Prediction, RaceResult (모듈 전환 시 추가); TrainerResult, JockeyResult 등 Kra 전환 시 추가
...
- **현재:** 스키마는 `docs/db/schema.sql` 수동 적용. `synchronize: false`.
...
- **스키마:** `docs/db/schema.sql` 수동 적용. TypeORM `synchronize: false`.
